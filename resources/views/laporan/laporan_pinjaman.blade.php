...
<body onload="window.print()">
    <div style="padding-left: 30px; padding-right: 30px">
        ...
        <div style="text-align: center">
            <h4>Laporan Pinjaman anggota <br>
        ...
        <div style="text-align: center">
            <h5 style="font-size: 18px;">LAPORAN PINJAMAN ANGGOTA</h5>
        </div>
        ...
        <div>
            ...
            <table border="1" style="width: 100%;border-collapse: collapse;font-size: 11px">
                <thead>
                 ...
                 <tbody>
                    @foreach($datapinjaman as $index => $item)
                    <tr>
                        <td> {{ $loop->iteration }} </td>
                        <td> {{ $item->user->nama }} </td>
                        <td> {{ $item->PenganjuanPinjamans->id }} </td>
                        <td> {{ $item->tanggal }} </td>
                        <td>Rp {{number_format($item->nominal,0,',','.')  }} </td>
                        <td>Rp {{ number_format($item->total,0,',','.')  }} (+10% bunga) </td>
                    </tr>
                    @endforeach
            </tbody>
            </table>
